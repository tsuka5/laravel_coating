                                ...
                                @foreach ($expiredUsers as $user)
                                    <tr>
                                        ...
                                        <form id="delete_{{ $user->id }}" method="post" action="{{ route('admin.expired-users.destroy', ['user' => $user->id ])}}">
                                            @csrf
                                            <td class="px-4 py-3">
                                                <a href="#" data-id="{{ $user->id }}" onclick="deletePost(this)" class="text-white bg-red-400 border-0 py-2 px-4 focus:outline-none hover:bg-red-500 rounded">Delete Completely</a>
                                            </td>
                                        </form>
                                    </tr>
                                @endforeach
