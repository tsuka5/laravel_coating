...
class SearchController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($materialName)) {
            // 'materialCompositions.material.materialDetail' というネストしたリレーションを辿る
            $query->whereHas('material_composition.material.material_detail', function ($q) use ($materialName) {
                $q->where('name', 'LIKE', "%{$materialName}%");
            });
        }

        // 「果物名」での絞り込み
        if (!empty($fruitName)) {
            // 'storingTests.fruitDetail' というネストしたリレーションを辿る
            $query->whereHas('storing_test.fruit_detail', function ($q) use ($fruitName) {
                $q->where('name', 'LIKE', "%{$fruitName}%");
            });
        }

        // 4. 全体を横断するキーワード検索 (OR条件)
        if (!empty($keyword)) {
            $query->where(function ($q) use ($keyword) {
                // Experiment自身のカラムを検索 (例: title)
                $q->orWhere('title', 'LIKE', "%{$keyword}%");

                // 関連テーブルの名前を横断して検索
                $q->orWhereHas('material_composition.material.material_detail', function ($subQuery) use ($keyword) {
                    $subQuery->where('name', 'LIKE', "%{$keyword}%");
                })
                    ->orWhereHas('storing_test.fruit_detail', function ($subQuery) use ($keyword) {
                        $subQuery->where('name', 'LIKE', "%{$keyword}%");
                    });
            });
        }

        // 5. Eager-Loadingとページネーション
        // ネストしたリレーションもドットつなぎで指定して、まとめて読み込む
        $selected_experiments = $query->with([
            'material_composition.material.material_detail',
            'storing_test.fruit_detail'
        ])->latest()->paginate(5);

        // 6. 検索フォームのプルダウン用リストを取得
        $materials_list = Material_detail::orderBy('name')->get();
        $fruits_list = Fruit_detail::orderBy('name')->get();
    ...
    public function experimentIndex($experiment_id)
    {
        ...
        return view('user.search.experiment_show', compact(
            'experiment',
            'materials',
            'compositions',
            'storing_test',
    ...
    public function show($id)
    {
        ...
        return view('user.search.show', compact(
            'experiment',
            'materials',
            'film_conditions',
            'charactaristic_tests',
            'storing_tests',
            'antibacteria_tests',
            'notes'
        ));
